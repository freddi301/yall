...
export type PropertiesLens<P, T> = Lens<P, T> & { [K in keyof T]: PropertiesLens<P, T[K]> };

export const properties = <T>(): PropertiesLens<T, T> => propertiesRecursive(identity());

export const propertiesRecursive = <P, T>(p: Lens<P, T>): PropertiesLens<P, T> =>
  new Proxy(p, {
    get(o, k) {
      if (k === get || k === set) {
        return p[k];
